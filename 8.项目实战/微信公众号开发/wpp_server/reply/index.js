...
// 引入工具模块
const {getBodyAsync, parseXMLAsync, formatJsMsg} = require('../utils')
// 引入处理发送数据模块
...
module.exports = () => async (req, res, next) => {
    ...
    if (req.method === 'GET') {
        ...
        if (sha1Str === signature) {
            ...
            // 流体接收数据
            const xmlMessageData = await getBodyAsync(req)
            // 将数据转换为js对象
            ...
            // 判断该消息是否存在:不存在返回错误
            if (!formatJsMessageData.MsgType) { res.send('error');return }


            // 获取处理后需要发送的数据
            const sendOptions = await parseSend(formatJsMessageData)
            // 将数据转换为xml模板
