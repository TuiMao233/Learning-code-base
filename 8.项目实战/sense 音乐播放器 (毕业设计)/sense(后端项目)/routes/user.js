...
router.post('/add_my_song', async ctx => {
    ...
    // 向数据库歌单列添加歌曲
    const upDateDoc = await findDoc.updateOne({ $addToSet: { song_list: songItem } })
...
router.post('/remote_my_song', async ctx => {
    ...
    // 向数据库歌单列删除该歌曲名称的项
    const upDateDoc = await findDoc.updateOne({ $pull: { song_list: { audio_name } } })
    console.log(upDateDoc)
    ctx.body = { code: 0, msg: '歌曲删除成功' }
})
