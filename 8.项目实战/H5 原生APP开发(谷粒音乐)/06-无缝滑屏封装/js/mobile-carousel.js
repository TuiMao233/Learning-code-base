...
const touchMiCarousel = new reusableDrag({
    ...
    touch: {
        start: function (ev) {
            ...
            // 保存按下时轮播图距离浏览器的偏移量
            this.startOffsetX = this.wrapper.getBoundingClientRect().x - ev.touches[0].clientX
            // 保存按下X/Y偏移量
        ...
        move: function (ev) {
            ...
            //如果Y的距离大于X，则直接返回 (防抖动)
            if(Math.abs(this.slidingCountY) > Math.abs(this.slidingCountX)) { return}
            // 手指位置 + 手指点击时轮播图容器距离浏览器的偏移量 = 轮播图容器偏移量
            var wrapper_slidingsetX = ev.touches[0].clientX + this.startOffsetX
